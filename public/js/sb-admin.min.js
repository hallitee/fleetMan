...
function getWarning(){
		m = document.querySelector("marquee");	
		h = document.querySelector("div h5");
		d = document.getElementById("dispWarning");
		if(disp.length>0){
		setInterval(()=>{
		if(cnt<disp.length){
		t = disp[cnt];
		d.style.display = "block";
		m.innerText = `${cnt+1}. ${t.renewmaster.name} for ${t.fleet.make} ${t.fleet.model}  ${t.fleet.regNo} almost reaching expiry ${moment(t.nextSub).fromNow()}`;
			//console.log("Within if "+cnt); 
		h.append(m);
		cnt++;			
